...
import { getHelpCategoryCard, getHelpHubListTitle } from '@/data/codelumeHelpContent'
  ...
  return (
    <div className="min-h-screen bg-background text-foreground">
      ...
      <main className="container mx-auto px-6 sm:px-8 lg:px-12 py-12 max-w-5xl">
        <h1 className="text-3xl sm:text-4xl font-black mb-10">{getHelpHubListTitle(t('help:title'))}</h1>

        <div className="grid gap-6 md:grid-cols-3">
          {HELP_DOC_KEYS.map((key) => {
            const card = getHelpCategoryCard(key, t(`help:categories.${key}.title`), t(`help:categories.${key}.description`))
            return (
              <Link
                key={key}
                to={`/help/${key}`}
